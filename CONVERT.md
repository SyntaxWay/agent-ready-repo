# Convert Any Repo to the AI-Ready Monorepo Structure

Copy the prompt below and paste it into any AI coding tool (Claude Code, Cursor, Codex, Gemini CLI, Windsurf, Cline, etc.) while inside the repo you want to convert.

**Claude Code users:** run `/migrate` instead — it's the same prompt as a slash command.
...
Wait for my reply before doing anything else.
...
**Dependency rule:** [derive from the project or use: apps/* may depend on
...
[List frozen paths, production infra dirs, legacy code scheduled for removal — infer from
.github/CODEOWNERS, existing comments in code, or ask me if none found.]
...
CURSOR — write .cursor/rules/agents.mdc:
  ...
  ---
  See [AGENTS.md](../../AGENTS.md) — single source of truth for all AI agents.
  Cursor-specific notes (only add things that apply ONLY to Cursor):
  - [any Cursor-specific tips found in existing .cursorrules, or leave empty]

GITHUB COPILOT — write .github/copilot-instructions.md:
  # GitHub Copilot Instructions
  See [AGENTS.md](../AGENTS.md) — single source of truth for all AI agents.
  Copilot-specific notes (only add things that apply ONLY to Copilot):
  - [any tips found in existing copilot-instructions.md, or leave empty]

GEMINI CLI — write GEMINI.md:
  # GEMINI.md
  See [AGENTS.md](./AGENTS.md) — single source of truth for all AI agents.
  Gemini-specific notes (only add things that apply ONLY to Gemini):
...
WINDSURF — write .windsurf/rules/agents.md:
  ...
  - [any Windsurf-specific tips, or leave empty]
...
If a file exists but is just a placeholder, update it with real content from Phase 1.
...
docs/architecture/system-overview.md
  ...
  ─ Include a Mermaid diagram if you can infer the topology from the code

docs/decisions/0000-adr-template.md
  ─ ADR template (copy from template repo)

docs/guides/README.md
  ─ index of guides with one-line descriptions

docs/guides/coding-standards.md
  ─ full coding conventions (expand the summary from AGENTS.md §6)

CONTRIBUTING.md
  ─ branch naming, commit message style, PR process, AI agent policy
  ─ only create if missing; don't overwrite an existing one

.github/PULL_REQUEST_TEMPLATE.md
  ─ checklist that matches AGENTS.md §8 Definition of Done

─────────────────────────────────────────────
PHASE 6 — CLEAN UP OLD AI TOOL FILES
─────────────────────────────────────────────

Delete every AI tool file for tools NOT in the confirmed list from Phase 2.

Delete map:
  Claude Code removed  → delete CLAUDE.md, .claude/ (settings.json only — keep other .claude/ content)
  Cursor removed       → delete .cursor/rules/agents.mdc  (delete .cursor/ if now empty)
  Copilot removed      → delete .github/copilot-instructions.md
  Gemini removed       → delete GEMINI.md
  Windsurf removed     → delete .windsurf/rules/agents.md  (delete .windsurf/ if now empty)
  Cline removed        → delete .clinerules
  Aider removed        → delete .aider.conf.yml

Also delete any legacy single-file formats that were replaced:
...
Print a report in this format:
  ...
  ✏️  Updated:   [list every modified file with one-line note on what changed]
  🗑️  Deleted:   [list every deleted file]
  ⏭️  Skipped:   [list files that already existed with real content and were left alone]

  Next steps for you:
  1. Fill in the placeholders in AGENTS.md (marked with [brackets]).
  2. Add domain terms to docs/GLOSSARY.md from your database schema.
  3. Write ADRs in docs/decisions/ for major decisions already made.
  4. Copy apps/example-app/AGENTS.md into each real app and customise it.
